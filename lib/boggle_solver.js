...
BoggleSolver.prototype._findWords = function(word, row, column) {
  ...
  for (i = 0; i < validCoordsForWord.length; i++) {
    ...
    if (this.dictionary._isValidPrefix(word.letters + this.boggleBoard.get(coords[ROW], coords[COL]))) {
...
BoggleSolver.prototype._isValidWord = function(word) {
  return word.letters.length >= this.minWordLength && this.dictionary._isValidWord(word.letters);
}
